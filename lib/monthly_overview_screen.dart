...
class MonthlyOverviewScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final dailyActivity = <int, Map<String, double>>{};
    for (var t in transactions) {
      final day = t.date.day;
      dailyActivity.putIfAbsent(day, () => {'income': 0, 'expense': 0});
      if (t.type == TransactionType.income) {
        dailyActivity[day]!['income'] = dailyActivity[day]!['income']! + t.amount;
      } else {
        dailyActivity[day]!['expense'] = dailyActivity[day]!['expense']! + t.amount;
      }
    }

    final barChartGroups = dailyActivity.entries.map((entry) {
      return BarChartGroupData(
        x: entry.key,
        barRods: [
          BarChartRodData(toY: entry.value['income']!, color: Colors.green, width: 8),
          BarChartRodData(toY: entry.value['expense']!, color: Colors.red, width: 8),
        ],
      );
    }).toList();
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const Text('December 2025', style: TextStyle(color: Colors.grey)),
            ...
            const Text('THIS MONTH FLOW', style: TextStyle(fontWeight: FontWeight.bold, color: Colors.grey)),
            const SizedBox(height: 10),
            GridView.count(
              shrinkWrap: true,
              physics: const NeverScrollableScrollPhysics(),
            ...
            const SizedBox(height: 20),
            const Text('Daily Activity', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 18)),
            const SizedBox(height: 10),
            SizedBox(
              height: 200,
              child: BarChart(
                BarChartData(
                  ...
                  titlesData: const FlTitlesData(leftTitles: AxisTitles(sideTitles: SideTitles(showTitles: false)), topTitles: AxisTitles(sideTitles: SideTitles(showTitles: false)), bottomTitles: AxisTitles(sideTitles: SideTitles(showTitles: true))), 
                ),
              ),
            ),
            const SizedBox(height: 20),
